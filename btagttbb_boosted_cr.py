...
for i in range(0,len(runs)):
    inFile = ROOT.TFile.Open('/Users/zefranrozario/Desktop/ttHbb_legacy/boosted/ttbb_PP8_mc16'+str(runs[i])+'.root')
    ...
    for ievt in range(0,tree.GetEntries()):	
        ...
        lightjets=[]
        cr_list=[]
        if tree.L2_DeepSets_tth_fraction<0.05:
            #cr_list.append(tree.L2_DeepSets_ttc_score)
            #cr_list.append(tree.L2_DeepSets_tt_1b_score)
            #cr_list.append(tree.L2_DeepSets_tt_2b_score)
            #cr_list.append(tree.L2_DeepSets_ttB_score)
            #cr_list.append(tree.L2_DeepSets_tt_light_score)
            #max_value=max(cr_list)
            #if cr_list.index(max_value) == 2:
                for j in range(0,tree.jet_pt.size()):
                    ptjet = tree.jet_pt[j]/1000
                    jetpt.Fill(ptjet,weight)
                    if tree.boostedRecoHiggsPt>300000 and tree.jet_truthflav[j]==5:
                        bjets.append(tree.jet_pt[j])
                        jetpt_b.Fill(ptjet,weight)
                        #jetpt_total.Fill(ptjet,weight)
                        passjetsel_b=True
                    elif tree.boostedRecoHiggsPt>300000 and tree.jet_truthflav[j]==4:
                        cjets.append(tree.jet_pt[j])
                        jetpt_c.Fill(ptjet,weight)
                        #jetpt_total.Fill(ptjet,weight)
                        passjetsel_c=True
                    elif tree.boostedRecoHiggsPt>300000 and ((tree.jet_truthflav[j]==0) or (tree.jet_truthflav[j]==15)):
                        lightjets.append(tree.jet_pt[j])
                        jetpt_light.Fill(ptjet,weight)
                        #jetpt_total.Fill(ptjet,weight)
                        passjetsel_light=True
                        
                if passjetsel_b==True and tree.boostedRecoHiggsPt>300000:
                    eventhist_b.Fill(bjets[0]/1000,weight)
                    eventpt_total.Fill(bjets[0]/1000,weight)
                    
                    
                if passjetsel_c==True and tree.boostedRecoHiggsPt>300000:
                    eventhist_c.Fill(cjets[0]/1000,weight)
                    eventpt_total.Fill(cjets[0]/1000,weight)
                    
                if passjetsel_light==True and tree.boostedRecoHiggsPt>300000:
...
outHistFile = ROOT.TFile.Open("jetpttureb_ttbb_boosted_cr.root" ,"RECREATE")
jetpt_b.Write()
eventhist_b.Write()
jetpt_c.Write()
eventhist_c.Write()
