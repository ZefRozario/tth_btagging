...
for i in range(0,len(runs)):
    ...
    for ievt in range(0,tree.GetEntries()):	
        ...
        for j in range(0,tree.jet_pt.size()):
            ...
            if 300000<tree.boostedRecoHiggsPt<450000 and tree.jet_truthflav[j]==5:
                bjets.append(tree.jet_pt[j])
                jetpt_b.Fill(ptjet,weight)
                #jetpt_total.Fill(ptjet,weight)
                passjetsel_b=True
            elif 300000<tree.boostedRecoHiggsPt<450000 and tree.jet_truthflav[j]==4:
                cjets.append(tree.jet_pt[j])
                jetpt_c.Fill(ptjet,weight)
                #jetpt_total.Fill(ptjet,weight)
                passjetsel_c=True
            elif 300000<tree.boostedRecoHiggsPt<450000 and ((tree.jet_truthflav[j]==0) or (tree.jet_truthflav[j]==15)):
                lightjets.append(tree.jet_pt[j])
                jetpt_light.Fill(ptjet,weight)
                #jetpt_total.Fill(ptjet,weight)
                passjetsel_light=True
                
        if passjetsel_b==True and 300000<tree.boostedRecoHiggsPt<450000:
            eventhist_b.Fill(bjets[0]/1000,weight)
            eventpt_total.Fill(bjets[0]/1000,weight)
            
            
        if passjetsel_c==True and 300000<tree.boostedRecoHiggsPt<450000:
            eventhist_c.Fill(cjets[0]/1000,weight)
            eventpt_total.Fill(cjets[0]/1000,weight)
            
        if passjetsel_light==True and 300000<tree.boostedRecoHiggsPt<450000:
...
outHistFile = ROOT.TFile.Open("jetpttureb_ttH_STXS_5.root" ,"RECREATE")
jetpt_b.Write()
eventhist_b.Write()
jetpt_c.Write()
eventhist_c.Write()
